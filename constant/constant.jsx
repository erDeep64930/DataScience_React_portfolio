// import img1 from "../public/images1/id.jpg";
// import img2 from "../public/images1/sm.jpg";
// import img3 from "../public/images1/design.jpg";


export const hambergerMenu =
    [
        { title: "HOME", link: "/" },
        { title: "SERVICE", link: "/service" },
        { title: "ABOUT", link: "/about" },
        { title: "CONTACT", link: "/contact" },
        { title: "PROJECT", link: "/project" },
        { title: "SKILL", link: "/skill" },
    ];
export const navMenu1 = [
    {
      id: 1,
      name: "Home",
      path: "/",
    },
    {
      id: 2,
      name: "About",
      path: "/about",
    },
    {
      id: 3,
      name: "Skills",
      path: "/service",
    }

]

export const navMenu2 = [
    {
      id: 1,
      name: "Projects",
      path: "/project",
    },
    {
      id: 2,
      name: "Contact",
      path: "/contact",
    },
    {
        id: 2,
        name: "Services",
        path: "/services",
      },
  ];
  
export const infoMenu = [
  {
    id:1,
    title:"Birthday",
    description:"9-March-1995",
  },
  {
    id:2,
    title:"Gender",
    description:"Male",
  },
  {
    id:3,
    title:"Email",
    description:"erdeep64930@gmail.com",
  },
  {
    id:4,
    title:"Address",
    description:"Ashok Vihar, New Delhi 110052",
  },
  {
    id:5,
    title:"Mobile",
    description:"+91-9939183692",
  },
  {
    id:6,
    title:"Driving License",
    description:"JH12-2020-000-2428",
  },
  {
    id:7,
    title:"Blood Group",
    description:"B (+ve)",
  },
  {
    id:8,
    title:"Nationality",
    description:"Indian",
  },
  {
    id:9,
    title:"Religion",
    description:"Hindu",
  },
  
] 

export const eduInfo =[
 {
  id:1,
  title:"Graduation",
  desc:"B.Tech",
  sub:"Branch",
  stream:"Electronics & Communication",
  desTitle:"Name of Institute",
  collName:"UCET",
  year:"Year of Passing",
  date:2018
 },

 {
  id:2,
  title:"Graduation",
  desc:"HSC(12)",
  sub:"Branch",
  stream:"I.Sc",
  desTitle:"Name of Institute",
  collName:"Bokaro Public School",
  year:"Year of Passing",
  date:2013
 },

 {
  id:3,
  title:"Graduation",
  desc:"SSC(10)",
  sub:"Branch",
  stream:"Matriculation",
  desTitle:"Name of Institute",
  collName:"Saint Clare School",
  year:"Year of Passing",
  date:2011
 },

 
 
]

export const serviceInfo = [
  {
    id:1,
    pic:"/dj.jpg",
    title:"Frontend Development",
    desc:"Web Development",

  },
  {
    id:2,
    pic:"/design.jpg",
    title:"React Developer",
    desc:"Web Development",

  },
  {
    id:3,
    pic:"/sm.jpg",
    title:"UI/UX Developer",
    desc:"Web Development",

  },
]
export const frontendTech = [
  {
    id: 1,
    src: "/html.png",
    title: "HTML",
    style: "shadow-orange-500",
  },
  {
    id: 2,
    src: "/css.png",
    title: "CSS",
    style: "shadow-blue-500",
  },
  {
    id: 3,
    src: "/javascript.png",
    title: "JavaScript",
    style: "shadow-yellow-500",
  },
  {
    id: 4,
    src: "/react.png",
    title: "React",
    style: "shadow-blue-600",
  },
  {
    id: 5,
    src: "/tailwind.png",
    title: "Tailwind",
    style: "shadow-sky-400",
  },
  {
    id: 6,
    src: "/nextjs.png",
    title: "Next JS",
    style: "shadow-white",
  },
  {
    id: 7,
    src: "/graphql.png",
    title: "GraphQL",
    style: "shadow-pink-400",
  },
  {
    id: 8,
    src: "/github.png",
    title: "GitHub",
    style: "shadow-gray-400",
  },
];

export const apps = [
  {
      id: "0",
      title: "Figma",
      icon: "/figma.png",
      width: 26,
      height: 36,
     
  },
  {
      id: "1",
      title: "Html",
      icon: "/html.png",
      width: 34,
      height: 36,
    
  },
  {
      id: "2",
      title: "CSS",
      icon: "/css.png",
      width: 36,
      height: 28,
     
  },
  {
      id: "3",
      title: "Github",
      icon: "/github.png",
      width: 34,
      height: 35,
    
  },
  {
      id: "4",
      title: "Express.js",
      icon: "/Express.png",
      width: 34,
      height: 34,
     
  },
  {
      id: "5",
      title: "JavaScript",
      icon: "/javascript.png",
      width: 34,
      height: 34,
     
  },
  {
      id: "6",
      title: "React",
      icon: "/react.png",
      width: 26,
      height: 34,
    
  },
  {
      id: "7",
      title: "Tailwind",
      icon: "/tailwind.png",
      width: 38,
      height: 32,
     
  },
];
export const testimonials = [
  {
    name: "John Doe",
    text: "This service is fantastic! I couldn't be happier.",
    image: "https://via.placeholder.com/100"
  },
  {
    name: "Jane Smith",
    text: "A wonderful experience! Highly recommend to everyone.",
    image: "https://via.placeholder.com/100"
  },
  {
    name: "Alice Johnson",
    text: "Excellent customer support and a great product.",
    image: "https://via.placeholder.com/100"
  }
];

export const dataScienceInfo =[
  {
    id:1,
    desc:"Data Science is the field focused on deriving valuable insights from raw data. It leverages a range of scientific techniques, algorithms, and processes to analyze large datasets. By applying these tools, data scientists can work with data from diverse sources, like financial records, multimedia, marketing data, sensor outputs, and text. A key part of Data Science involves preparing data for analysis, which includes steps like cleaning, combining, and transforming data to enable complex analyses and support data-driven decision-making."
  },
  {
    id:2,
    desc:"A Data Scientist is a skilled professional who uses math, statistics, and computer science to analyze data and uncover insights that help businesses make informed decisions. They collect data from various sources, work with both structured and unstructured data, and develop strategies to solve business challenges. Data Scientists collaborate with team members and leaders to create effective data strategies and use algorithms to find patterns and trends. They also ensure the data is clean, accurate, and consistent, transforming complex information into actionable insights that support better decision-making."
  }
]


export const dsInfo =[
  {
    id:1,
      dp:"/figma.png" ,
     
  },
  {
    id:2,
      dp:"/excel.svg" ,
     
  },
  {
    id:3,
      dp:"/flask.svg" ,
     
  },
  {
    id:4,
      dp:"/github.png" ,
     
  },
  {
    id:5,
      dp:"/jupiter.svg" ,
     
  },
  {
    id:6,
      dp:"/matplotlib.webp" ,
     
  },
  {
    id:7,
      dp:"/mongodb.svg" ,
     
  },
  {
    id:8,
      dp:"/nextjs.png" ,
     
  },
  {
    id:9,
      dp:"/node.png" ,
     
  },
  {
    id:10,
      dp:"/numpy.svg" ,
     
  },
  {
    id:11,
      dp:"/python.svg" ,
     
  },
  {
    id:12,
      dp:"/pandas.svg" ,
     
  },
  {
    id:13,
      dp:"/pytorch.svg" ,
     
  },
  {
    id:14,
      dp:"/tableau.svg" ,
     
  },
]


export  const servicesMenu = [
  {id: 1, name:'UI & UX DESIGNING', description:'I design beautiful web iterfaces with Figma and Adove XD', image:'/award.svg'},
  {id:2, name:'WEB DEVELOPMENT', description:'I create beautiful iterfaces with simple HTML, CSS, & JavaScript and also frameworks like Angular and ReactJS', image:'/award.svg'},
  {id:3, name:'MOBILE DEVELOPMENT', description:'I am an expert mobile developer. I have experience using Flutter and React Native.', image:'/award.svg'},
  {id:4, name:'VERSION CONTROL', description:'I can use version control systems well, and Git & Mecurial are my go-to tool.', image:'/award.svg'},
  {id:5, name:'NPM AND NODEJS', description:'I have core understanding of NPM. I can also develop general purpose applications with NodeJS', image:'/award.svg'},
  {id:6, name:'WEB SCRAPING', description:'I can collect content and data from the internet then manipulate and analyze as needed.', image:'/award.svg'},
]


export const skillMenu = [
  { name: "Website Development", image:"/tech.jpg"  },
  { name: "Logo Design ", image: "/tech.jpg"},
  { name: "Digital marketting", image:"/tech.jpg" },
  {
    name: "Content Writting",
    image: "/tech.jpg",
  },
];